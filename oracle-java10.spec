...
%define		bld_ver	13
%define		bhash	19aef61b38124481863b1413dce1855f
...
# NoSource0-md5:	1cc0fa1d7f059978ae22a688dabcdc0b
NoSource:	0
Source1:	Test.java
Source2:	jmc.desktop
...
%define		javareldir	java10-%{version}
%define		javadir		%{_jvmdir}/%{javareldir}
%define		jvmjardir	%{_jvmjardir}/java10-%{version}
...
java10-jdk-base to system-wide directories like /usr/bin, making Oracle
...
java10-jdk-base, w standardowych systemowych ścieżkach takich jak
...
by java10-jre-base to system-wide directories like /usr/bin, making
Oracle Java the default JRE.

%description jre -l pl.UTF-8
Ten pakiet tworzy symboliczne dowiązania do narzędzi środowiska
uruchomieniowego Javy firmy Oracle, dostarczanych przez pakiet
java10-jre-base, w standardowych systemowych ścieżkach takich jak
...
java10-jre-base-X11 to system-wide directories like /usr/bin, making
...
Oracle, dostarczanych przez pakiet java10-jre-base-X11, w standardowych
...
for i in java jjs keytool orbd javaws \
...
for i in appletviewer idlj jaotc jar jarsigner \
	javac javadoc javap javapackager jcmd jconsole jdb jdeprscan jdeps jhsdb jimage jinfo jlink \
...
ln -s %{javareldir} $RPM_BUILD_ROOT%{_jvmdir}/java10
ln -s java10-%{version} $RPM_BUILD_ROOT%{_jvmjardir}/java
ln -s java10-%{version} $RPM_BUILD_ROOT%{_jvmjardir}/jre

# ugly hack for libavplugin.so
cp -p -n $RPM_BUILD_ROOT%{javadir}/lib/libavplugin-57.so \
	$RPM_BUILD_ROOT%{javadir}/lib/libavplugin-58.so
%{__sed} -i -e '
	s#\.so\.57#.so.58#g
...
rm $RPM_BUILD_ROOT%{javadir}/lib/libavplugin-57.so
rm $RPM_BUILD_ROOT%{javadir}/lib/libavplugin-ffmpeg-56.so
rm $RPM_BUILD_ROOT%{javadir}/lib/libavplugin-ffmpeg-57.so
...
# new javac: RPATH=%{_prefix}/lib/jvm/java10-1.6.0/jre/lib/i386/jli
...
%exclude %{javadir}/lib/libavplugin-58.so
...
%files javafx
%defattr(644,root,root,755)
%attr(755,root,root) %{javadir}/lib/libavplugin-58.so
